...
Device identity, the wire codecs, the helper math, and the hardware a gateway actually talks to - serial packet framing, Modbus RTU, CAN and J1939, on-board I2C/SPI/GPIO, and the sensor and actuator drivers themselves - reach all three bindings alongside the MQTT transport, and so does the reach layer: LoRa airtime and duty-cycle budgeting, secured LoRaWAN frames with over-the-air activation, mesh packets, and cost-aware routing. A reading can be pulled off an RS485 sensor or a BME280, smoothed, signed, packed for a metered link, and sent kilometres over LoRaWAN or hopped across a mesh, with a servo or stepper driven in reply, without leaving the language you started in. A single file of conformance vectors, generated from the Rust implementation, is asserted by every binding's test suite, so the four cannot quietly disagree about what the same call returns.
