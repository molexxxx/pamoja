...
pub fn render() -> Vec<(String, String)> {
    vec![
        ...
        ("docs/theme/header.js".to_owned(), header()),
...
// The mark in the site header, in place of mdBook's plain title, on every page. A
// script rather than a theme override, so the book is not pinned to one mdBook version,
// and an `img` rather than a CSS background, so the mark animates. Without scripting the
// header keeps the title text, which is the same word.
fn header() -> String {
    String::from(
...
.menu-title {{ display: flex; align-items: center; justify-content: center; }}
.menu-title a {{ display: inline-flex; }}
.pamoja-mark {{ height: 30px; width: auto; display: block; }}
.pamoja-landing table {{ width: 100%; }}
...
mod tests {
    ...
    fn every_adapter_carries_the_palette_and_names_its_generator() {
        for (path, body) in render() {
            ...
            // The header script carries behaviour rather than colour.
    ...
    #[test]
    fn the_header_script_puts_the_mark_in_place_of_the_title() {
        let files = render();
        let script = &files
            .iter()
            .find(|(path, _)| path.ends_with("header.js"))
            .expect("a header script")
            .1;
        assert!(script.contains("menu-title") && script.contains("pamoja-logo.svg"));
    }
