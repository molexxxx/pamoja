...
/// given to the process, never generated by it and printed. Until somebody provisions
/// that secret the console is read-only, which is the right default for a page whose job
/// is to draw a fleet: nothing reaches stdout that would unlock it if the output were
/// captured into a log. `gateway.rs` next door is the other case, one node an operator
/// pairs at a terminal, and it shows a generated code once for exactly that.
