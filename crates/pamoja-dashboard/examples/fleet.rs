...
use pamoja_dashboard::{
    Assets, Auth, Catalog, Command, Fleet, LinkKind, Reading, Sensor, Server, Status,
};
...
///
/// A pairing code unlocks control of the whole fleet, so showing a generated one is
/// right at a terminal an operator reads once and wrong under a service manager, where
/// stdout goes into a log anyone with log access can read.
fn pairing_secret() -> Option<String> {
    if let Ok(provisioned) = std::env::var(SECRET_VAR) {
    ...
    if std::io::stdout().is_terminal() {
        println!("fleet: pairing code (unlock control with this): {secret}");
        return Some(secret);
    }
    eprintln!("fleet: no terminal to show a pairing code on, so control stays locked.");
    None
}
