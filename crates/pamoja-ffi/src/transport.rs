...
pub unsafe extern "C" fn pamoja_transport_faulty(
    ...
    PamojaTransport::into_raw(Kind::Faulty(Box::new(pamoja_loopback::Faulty::new(
        inner, failures,
    ))))
}
...
pub unsafe extern "C" fn pamoja_transport_degraded(
    ...
    let mut link = pamoja_sim::DegradedLink::new(inner);
    if drop_every != 0 {
    ...
    PamojaTransport::into_raw(Kind::Degraded(Box::new(link)))
}
