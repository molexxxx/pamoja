// The bus layer guide example; see docs/guides/hal.md.
...
function check(seen: {
  humidity: number
  asleep: boolean
  reading: Bme280Measurement
  cold: Bme280Measurement
  waited: number
  checked: Bme280Measurement
  script: I2cBus
}): void {
  assert.equal(seen.humidity, bme280.oversampling.x1)
  assert.equal(seen.asleep, true)
  assert.equal(seen.reading.celsius.toFixed(2), '20.44')
  assert.equal(seen.reading.pascals, 84_805)
  assert.equal(seen.cold.celsius, 4)
  assert.ok(Math.abs(seen.cold.hectopascals - 1013.25) < 0.01)
  assert.equal(seen.waited, 2_000 + 2 * 9_300)
  assert.deepEqual(seen.checked, seen.reading, 'the same part, read two ways')
  assert.equal(seen.script.transfers, 11)
  assert.equal(seen.script.remaining, 0)
}
