 ...
 * `I2cBus.simulated([parts])` puts {@link I2cPart}s on a bus, each answering at its own
 * address, and `I2cBus.scripted([steps])` plays {@link I2cStep}s in order and refuses any
 ...
 * the kernel's own words. `part(address)` copies what a simulated part holds now,
 * `transfers` counts every transfer, `remaining` is what a script has left, and
