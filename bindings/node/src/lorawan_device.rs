...
pub struct LorawanDeviceSettings {
    ...
    pub min_output_dbm: checked::i32,
    /// The highest, conducted, in dBm.
    pub max_output_dbm: checked::i32,
    ...
    pub antenna_gain_db: Option<checked::i32>,
    /// The lowest frequency the radio and its front end can use, in hertz.
    pub lowest_hz: Option<checked::u32>,
    /// The highest, in hertz.
    pub highest_hz: Option<checked::u32>,
...
impl LorawanEndDevice {
    ...
    /// cannot tell.
    #[napi]
    pub fn set_battery(&mut self, battery: Option<Either<f64, String>>) -> Result<()> {
        let battery = match battery {
            None => Battery::Unknown,
            Some(Either::A(level)) => Battery::Level(checked::whole(level)?),
...
pub(crate) fn made_from(
    ...
    let min = byte(settings.min_output_dbm.get(), "minOutputDbm")?;
    let max = byte(settings.max_output_dbm.get(), "maxOutputDbm")?;
    let lowest_hz = settings.lowest_hz.get().unwrap_or(137_000_000);
    let highest_hz = settings.highest_hz.get().unwrap_or(1_020_000_000);
    ...
    let mut built = Settings::new(min, max)
        ...
        .with_antenna_gain(byte(
            settings.antenna_gain_db.get().unwrap_or(0),
            "antennaGainDb",
        )?)
        .with_tuning_range(lowest_hz, highest_hz)
        .with_seed(settings.seed.get().unwrap_or(0));
...
fn with_counters(
    ...
    match counters {
        Some(counters) => device.with_frame_counters(counters.up.get(), counters.down.get()),
        None => device,
    }
}
