...
src/Pamoja.Native/      Pamoja.Native: the P/Invoke contract (Interop/), mirroring the
                        generated C header, and the native library per runtime identifier;
                        every other package depends on it
src/Pamoja.Core/        Pamoja.Core: the engine's surface (the version, the exception, the
                        handle type, and the transport every link shares)
