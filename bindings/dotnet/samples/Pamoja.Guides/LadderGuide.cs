...
public static class LadderGuide
{
    ...
    /// <returns>A task that completes once a report has gone out between waits.</returns>
    public static async Task RunAsync()
    {
        ...
        Expect(first == Delivery.Sent, "the harbor took the first report");
        Expect(stormy == Delivery.Buffered, "the storm buffered the fourth");
        Expect((waiting, still, left) == (1, 1, 0), "the backlog went out exactly once");
        Expect(order.Text == "return to port", "the order came back over the ladder");
        Expect(last.Text == "report 5", "and a report went out between waits");
    }
}
