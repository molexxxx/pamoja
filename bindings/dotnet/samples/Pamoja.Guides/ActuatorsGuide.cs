...
public static class ActuatorsGuide
{
    ...
    public static void Run()
    {
        ...
        using (bus)
        {
            ...
            Expect(controller.Prescale == 121, "round(25 MHz / 4096 / 50) - 1");
            Expect(Math.Abs(controller.Frequency - 50.0288f) < 1e-3f, "25 MHz / (4096 * 122)");
            Expect(tilt.Off == 266, "1300 us of a 20 ms period in 4096 counts");
            Expect(status.Off == 256, "a sixteenth of the period");
            Expect(slide == 400, "an eighth of 3200 microsteps");
            Expect(panStep == 23, "2 degrees of 4096 half-steps, rounded");
            Expect(slider.Position == 1_200, "three moves of 400");
            Expect(pan.Position == 69, "three moves of 23");
            Expect(pulses == 1_200, "one rising edge a microstep");
            Expect(sliderDelay.TotalMicros == 1_200 * (10 + 10 + 500), "setup, pulse, interval");
            Expect(panDelay.TotalMicros == 69 * 4_000, "4 ms a half-step");
            Expect(stillHeld == 121, "the prescale written awake was dropped");
            Expect(fast == 5, "round(25 MHz / 4096 / 1000) - 1");
            Expect(allOff && asleep, "parked");
            Expect(
                parkedMode == (Pca9685.Mode1.AutoIncrement | Pca9685.Mode1.Sleep),
                "no channel was running, so RESTART stays clear");
            Expect(bus.WaitedMicros == 500, "the oscillator's one start-up");
        }
    }
}
