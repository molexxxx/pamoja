...
assert humidity == bme280.Oversampling.X1
assert asleep is True
assert f"{reading.celsius:.2f}" == "20.44"
assert reading.pascals == 84_805
assert waited == 2_000 + 9_300
assert cold.celsius == 4.0
assert abs(cold.hectopascals - 1013.25) < 0.01
assert bus.waited_micros == 2_000 + 2 * 9_300
assert (checked.celsius, checked.pascals) == (reading.celsius, reading.pascals)
assert script.transfers == 11
