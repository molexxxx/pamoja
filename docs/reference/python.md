...
The [`pamoja` reference](python/pamoja.html) is generated by pdoc from the Python
facade. Each capability is a module of the package, and the page for each one
lists its classes and functions with their signatures.

The facade is hand-written Python over the native extension `pamoja._core`,
whose type stub ships in the package as `pamoja/_core/__init__.pyi`, so an
editor sees every native signature; `pamoja.raw` re-exports the native contract
...
| Transports | `pamoja.transport` | The transport surface every link shares: send, receive, subscribe, and a faulty wrapper for tests |
